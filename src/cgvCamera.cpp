...
// TODO: Practice 2b.C: Modify this method in order to adequately apply a zoom to the camera.
    ...
    if (camType==CGV_PARALLEL) {
        glOrtho(xwmin*currentZoom, xwmax*currentZoom, ywmin*currentZoom, ywmax*currentZoom, znear+nearPlaneMod, zfar);
    }
    if (camType==CGV_PERSPECTIVE) {
        gluPerspective(fovy*currentZoom,aspect,znear+nearPlaneMod,zfar);
    }
...
/**
 * Update the parameters to apply a zoom
 * @param factor Zoom factor
 * @post update the currentZoom parameter
 */
...
void cgvCamera::alterPerspective(){
    if(camType==CGV_PARALLEL){
        camType= CGV_PERSPECTIVE;
    }
    else{
